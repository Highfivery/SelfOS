...
test('memory: a member sees only their OWN insights, never another member’s (the cross-user leak fix, spec 20 §1.1)', async () => {
  ...
  try {
    ...
    // Ana's Memory shows HER portrait (the hero narrative) — and NEVER Bo's (the cross-user leak).
    await w.getByRole('link', { name: 'Memory' }).click();
...
test('memory (#129): a questionnaire you sent a partner is grouped under "Responses", not mislabelled "About you"', async () => {
  ...
  try {
    ...
    // A genuine own insight is in its life-area SECTION (collapsed); the sent-questionnaire insight is NOT.
    ...
    // The sent-questionnaire insight lives in the "From questionnaires you sent" band, under Angel…
...
test('memory redesign (54/57): a partner’s shared facts never show raw; the Sharing & relationships page shows AI relationship insights', async () => {
  ...
  try {
    ...
    // Expanding the owner's section shows their OWN note…
    await w.getByRole('button', { name: /^Relationships/ }).click();
...
test('memory: the dashboard groups by life-area, flags a fact (decrypt-persisted), shows "source removed", and fits 390px (spec 20 §3)', async () => {
  ...
  try {
    ...
    // Approved insights are life-area SECTIONS (collapsed by default); the trends panel is present.
    await expect(w.getByRole('button', { name: /^Health & body/ })).toBeVisible();
...
test('memory overview: portrait hero, drill in to type-scope a fact to partner (decrypt), mark "not right" (decrypt), Edit answer, Sharing page, 360px (spec 44/57)', async () => {
  ...
  try {
    ...
    // The portrait narrative shows in the hero; the stats strip carries the confidence read.
    await expect(w.getByText('A grounded, curious person').first()).toBeVisible();
    await expect(w.getByText(/overall confidence/i)).toBeVisible();
    ...
    // Back to Memory → the portrait is the hero; "Edit your answers" deep-links into onboarding (§3.4).
    await w.getByRole('link', { name: 'Memory' }).click();
...
test('memory cleanup: flagging a previously-shared fact retracts it from a related person’s context (39 §4.2)', async () => {
  ...
  try {
    const w = await app.firstWindow();
    await w.getByRole('link', { name: 'Memory' }).click();
    // Expand the section where the per-fact correction control lives.
...
test('memory: a provenance link opens the live source (dream deep-link survives the per-person reset, spec 20 §3.3)', async () => {
  ...
  try {
    const w = await app.firstWindow();
    await w.getByRole('link', { name: 'Memory' }).click();
    // Expand the section where the provenance deep-link lives.
    await w.getByRole('button', { name: /^Emotions & patterns/ }).click();
...
test('questionnaires excerpt (§3.1): the Insight excerpt clamps whole lines, expands in place, and deep-links to the insight in Memory', async () => {
  ...
  try {
    ...
    // "View in Memory" deep-links to the insight — its section (the responses band) opens inline and the
    // full summary is on screen (the flattened page has no separate detail view — spec 62).
...
test('onboarding: nudge → turn fills a field → skip intimacy → portrait feeds context → owner sees restricted facts (18)', async () => {
  ...
  try {
    ...
    // The restricted 'grief' fact is shown to the Owner in the portrait card (reached via search).
...
test('memory redesign (62): sections collapsed (sensitive too), edit a fact inline, decrypt + 360px guard', async () => {
  ...
  await saveInsight(fs, key, {
    ...
    facts: [{ id: 'g1', text: 'A private detail', shareable: false, restricted: true }],
    confidence: 'medium',
    categories: ['Intimacy'],
    approved: true,
    provenance: { conversationId: 'c2', at: now },
    createdAt: now,
    updatedAt: now,
  });
  ...
  try {
    ...
    // The page opens with sections COLLAPSED — the sensitive (Intimacy) one stays closed.
    const emotions = w.getByRole('button', { name: /^Emotions & patterns/ });
    await expect(emotions).toHaveAttribute('aria-expanded', 'false');
...
test('story (64): living book — refresh proposes a structural change, approving restructures the outline; find-what’s-missing mints a check-in', async () => {
  ...
  try {
    ...
    // further AI: a "Worth telling next" heading + a life-map coverage bar (progressbar) with a visible coverage
    // word (the §9 text equivalent).
    await expect(w.getByRole('heading', { name: 'Worth telling next' })).toBeVisible();
    await expect(w.getByRole('progressbar').first()).toBeVisible();
    await expect(w.getByText(/richly told|taking shape|thin|not yet begun/).first()).toBeVisible();
...
test('74: the Dirty Talk take — mark, split, complete, and the boundary holds everywhere', async () => {
  ...
  try {
    ...
    // 74 §3.6.9 — the map. Opening the take used to say "What do you call each other?" with no indication
    ...
    for (let i = 0; i < 40; i += 1) {
      const done = w.getByRole('button', { name: /Done with the words →/ });
    ...
    await expect(w.getByRole('heading', { name: /does this land\?/i })).toBeVisible();
    // Only a handful marked, so the step that WRITES refuses rather than generating from nothing — and says
    // how much more it needs. Finishing is still one tap away: a thin profile beats being unable to finish.
    await expect(w.getByText(/Not enough marked yet/i)).toBeVisible();
    await w.screenshot({ path: 'e2e-artifacts/74-ai-step-blocked.png' });
    // Nothing generatable is left, so the rail's next step IS the profile — and it is rendered once, not as
    ...
    for (const name of insightNames) {
      ...
      if (json.includes('dirty-talk')) {
        found = true;
        expect(json).not.toContain('beg like the slut you are');
...
/**
 * 74 §3.6.9 — the take is navigable, and an AI step never spends on arrival.
 *
 * Both of these were reported. The take was a one-way chain: `setPhase` was called exactly twice in the whole
 * screen (both to return to the identity taps), so a resumed take dropped into whichever AI phase it had reached
 * with no route back to the person's own words. And every AI phase fired its call the moment it mounted — which,
 * once any step is a tap away, makes a mis-tap a billed request.
 */
test('74 §3.6.9: every step is reachable both ways, and an AI step waits to be asked', async () => {
  test.setTimeout(120_000);
  ...
  try {
    ...
    for (const label of [
      ...
      await expect(map.getByRole('button', { name: label })).toBeVisible();
    }
    // Nothing marked yet ⇒ the AI steps are disabled with their reason, not live taps into a paid call that
    // could only come back empty.
    const linesRow = map.getByRole('button', { name: /Lines written for you/i });
    await expect(linesRow).toBeDisabled();
    await expect(linesRow).toContainText(/marks first/i);
    await w.screenshot({ path: 'e2e-artifacts/74-map.png', fullPage: true });
    ...
    // The rail is on every step and carries the same seven rows as the map.
    const rail = w.getByRole('complementary', { name: /The steps/i });
    // In the rail the reason is the row's accessible name — as trailing text it squashed the labels it was
