...
describe('usageStore', () => {
  it('records, queries, and summarizes usage', async () => {
    ...
    const events = await queryUsage(vault, key, {
  ...
  it('stores usage encrypted at rest', async () => {
    await savePerson(vault, key, person('p1', 'Alex'));
    await recordUsage(vault, key, ev({ id: 'e1', model: 'claude-opus-4-8' }));
  ...
  it('filters by date range', async () => {
    await savePerson(vault, key, person('p1', 'Alex'));
    await recordUsage(vault, key, ev({ id: 'old', at: '2026-05-01T00:00:00.000Z' }));
    await recordUsage(vault, key, ev({ id: 'new', at: '2026-06-10T00:00:00.000Z' }));
    const june = await queryUsage(vault, key, {
      from: '2026-06-01T00:00:00.000Z',
