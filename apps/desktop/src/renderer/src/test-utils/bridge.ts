...
export function installMockBridge(overrides: Partial<SelfosBridge> = {}): SelfosBridge {
  const bridge: SelfosBridge = {
    ...
    questionnairesRemovePartnerWish: () =>
      Promise.resolve({
        candidates: [],
        areas: [],
        markedOff: [],
    ...
    assignmentsDismiss: () => Promise.resolve(),
    assignmentsCorrectFact: () =>
      Promise.resolve({ ok: true, rewrite: { prompt: 'Corrected question?' }, source: 'unknown' }),
