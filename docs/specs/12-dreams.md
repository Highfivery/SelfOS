...
- **dreamAnalysisService** — the guided-analysis orchestration. Reuses `05`'s streaming/turn + budget +
  ...
  instructions + `buildContext(personId, dreamTopic(dream, relTypes))` (the dream's life-areas — incl. the
  linked figures' relationship types, 28 §13.1) + **`buildLinkedPeopleContext`** — the shareable context of the
