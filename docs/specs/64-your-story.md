...
## 18. Backlog batch 3 — publisher-grade (2026-07-22, PROPOSED)

The fourth batch of the groomed backlog (GitHub #290, #300, #293, #303): the apparatus a book needs before it
could actually be published. Decisions locked with the owner 2026-07-22 (AskUserQuestion): **#290** manual
consent tracking + pseudonyms, warn-not-block at publish; **#293** EPUB + DOCX + a _generic_ Sources appendix;
**#303** regenerate the print cover at 2:3.

### 18.1 People-in-your-book consent center + pseudonyms (#290) — BUILT (B3-1)

**The change.** A per-book consent register of everyone the book names — drawn from the SAME source as the cast
register (People graph + memories + named mentions) — where the author tracks each person's consent **manually**
(`unknown` / `requested` / `granted` / `declined`; SelfOS never contacts anyone) and can assign a **pseudonym**.
The pseudonym map (`realName → pseudonym`) is applied everywhere the book is READ or EXPORTED, while the DRAFT
keeps the real name.

- Pure `storyConsent.ts`: `getConsentRegister` (cast joined with stored decisions, new people default `unknown`),
  `setConsentEntry`, `pseudonymMap`, `pseudonymizeChapters`/`pseudonymizeCast`/`pseudonymizeManifest`,
  `unconsentedNames`. `consent.enc` store (the exclusions/continuity precedent). `applyPseudonyms(text, map)` in
  `storyText.ts` — whole-word, case-insensitive, longest-name-first.
- **EVERY reader-facing name-bearing surface is covered — not just the prose.** `pseudonymizeChapters` substitutes
  the chapter TITLE + prose + pinned quotes + each image-placement caption; `pseudonymizeManifest` substitutes the
  title + AI essence + matter (dedication / epigraph / acknowledgments / about-the-author / colophon) + cast +
  image captions/vision-notes. An AI-written title / caption / essence is auto-generated, so the author never got
  to self-substitute — leaving any of these uncovered would leak a real name past the pseudonym.
- **Applied at three sites so every leak surface is covered:** `publishBook` **freezes** the substituted chapters
  (title/prose/quotes/captions) + the pseudonymized manifest (title/essence/matter/cast/captions) into the
  published head (a shared reader can't read the author's consent register); `readOwnBook` applies the same two
  helpers **live** (owner's own immersive reader); `readDraftHead` reuses `readOwnBook`, so both draft exports
  inherit it. The shared reader + published export read the already-substituted frozen head.
- **Warn, never block (§8.2 / owner decision):** the Share panel warns which named people would appear under
  their real name (not `granted`, no pseudonym), but publishing is never gated — a pseudonym is the fix.
- Seam: `story:consent` / `:setConsent` (gated `story.own` + active-person-scoped). UI: a "People in your book"
  consent center on the Settings tab (consent `Select` + pseudonym field per person) + the pre-publish warning.

### 18.2 Publish lifecycle — publish-diff + unpublish (#300) — BUILT (B3-2)

Republish silently shrunk the reader's book; adds a publish-diff preview (what readers gain/lose/see-updated),
an unpublish action, and a confirm against a silent shrink.

- **`computePublishDiff`** (pure/derived) diffs the current published head vs the next-publish set (Reviewed
  chapters in outline order — the same computation `publishBook` uses): `added` (readers gain), `removed`
  (readers LOSE — the shrink), `updated` (prose/title changed), `unchanged`, plus `willShrink` +
  `nothingToPublish`. The change comparison is done on the NEXT published FORM (draft prose run through the
  pseudonym map) vs the CURRENT published form (already pseudonymized), so a freshly-assigned pseudonym reads as
  an `updated` chapter (the reader would see a change) while a pseudonym that's already in the head reads as
  `unchanged` — never a phantom edit.
- **`unpublishBook`** clears `publishedAt` so the shared-read re-gate (`!book.publishedAt` → null) denies
  immediately — the SAME enforcement as revoke. It KEEPS the frozen head + the reader grant list, so a later
  re-publish restores exactly the same readers; the draft is untouched; idempotent when already unpublished.
- Seam: `story:publishDiff` (read) + `story:unpublish` (gated `story.own`, author-scoped). UI (Sharing tab): a
  diff preview banner (Add/Update/Remove named chapters) on a published book, an **Unshare** button behind a
  confirm, and a **no-silent-shrink confirm** — clicking "Share updates" when a re-publish would drop chapters
  readers have stops to ask ("Remove & share" / "Keep them") instead of silently shrinking.

### 18.3 Export parity + EPUB/DOCX + Sources appendix (#293)
...
numbers + trim size. **BUILT** across three sub-slices: **B3-3a** (Sources appendix + PDF page numbers/trim),
**B3-3b** (EPUB), **B3-3c** (DOCX) — #293 is complete.
...
**B3-3c — DOCX export (BUILT):**

- **`bookToDocx`** builds a valid Office Open XML `.docx` (reusing the ZIP writer): `[Content_Types].xml` + the
  package rels + `word/document.xml` (WordprocessingML) + a minimal `word/styles.xml` (Title/Heading1/2/Normal,
  Georgia) + image parts with DrawingML `<wp:inline>` references. Image display size is derived by reading the
  **PNG IHDR / JPEG SOF** header for pixel dimensions, scaled to a 5.5in max width (EMU) preserving aspect. Text
  reuses the escape-first helpers via a small `inlineRuns` tokenizer (`**bold**`/`*italic*` → `<w:r>` runs), over
  the pseudonymized manifest/chapters, and renders the same matter + Sources appendix + the §8.2 boundary.
- Seam: `story:exportDocx` (draft or published head), gated `story.own`, author-scoped; the ExportDialog gains a
  **Word** format option.

### 18.4 Print cover pipeline (#303) — BUILT (B3-4)

The cover was a fixed 1024×1024 **square** the UI CSS-cropped to 2:3, so a real print/export cover was
unreachable. Now the **cover renders natively at the 2:3 book trim** (`COVER_SIZE = '1024x1536'`, the largest
2:3 OpenAI offers) while chapter illustrations stay square (`1024x1024`, inline figures). The cover distillation
brief now asks for a **vertical-portrait composition**, and the CoverPanel preview shows the cover at `2 / 3`
(uncropped). Every export already lays the cover out at its true aspect — DOCX sizes it from the real PNG/JPEG
header (`imageDimensions`), EPUB/PDF/Markdown use `max-width` — so a 2:3 cover flows through unstretched.

**Honest caveat (resolution):** OpenAI's largest 2:3 render is 1024×1536, not the aspirational 1800×2700 print
resolution; the aspect + composition are correct at the best fidelity the provider offers. A native
high-resolution / bleed pipeline would need a different renderer and is out of scope. Existing square covers
stay square until regenerated (a regenerate produces the 2:3 cover).

### 18.5 Build slices (each its own PR, standard §6/§7 cadence)

| Slice   | Issue | Scope                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Tests                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ------- | ----- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| B3-1    | #290  | Pure `storyConsent.ts` (register, set, pseudonym map, `unconsentedNames`) + `applyPseudonyms`; `consent.enc` store; substitution FROZEN at publish + applied LIVE in `readOwnBook` (draft export inherits it); the `story:consent`/`:setConsent` seam; the Settings consent center + the warn-not-block pre-publish notice.                                                                                                                                 | Core: whole-word case-insensitive pseudonym replace (never mid-word), longest-first; register defaults unknown; set/clear pseudonym + `unconsentedNames`. Publish: a pseudonym is frozen into the published head — the shared reader never sees the real name, the draft keeps it. Bridge: register + set + refused without `story.own`. RTL: consent center sets a pseudonym → the warning clears. E2E: set a pseudonym → decrypt `consent.enc` + the warning clears. |
