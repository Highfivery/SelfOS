...
describe('analyzeAssignment', () => {
  ...
  it('a PRIVATE send sends the model the skip KIND but never the recipient’s words (08 §34.2)', async () => {
    const fs = memFileSystem();
    const q = await saveQuestionnaire(fs, key, {
      title: 'Check-in',
      type: 'role-feedback',
    ...
    expect(result.ok).toBe(true);
    const prompt = seen.join('\n');
    expect(prompt).toContain('How is your mother?'); // the sender's own question — safe
    expect(prompt).toContain('skipped it'); // the kind — safe
    expect(prompt).not.toContain('she died in March'); // her words — never
  });
